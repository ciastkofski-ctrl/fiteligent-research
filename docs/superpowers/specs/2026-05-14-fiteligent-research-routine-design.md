# Fiteligent Research Routine — Design Spec

**Date:** 2026-05-14
**Project:** `fiteligent-research/`
**Owner:** Dr. Jakub Stanisławski (fiteligent.pl)
...
1. **`digest.md`** — English, technical, theme-grouped research review (~4000 words, 15–25 studies), with evidence-strength ratings. For Dr. Stanisławski's personal intake and as primary reference material.
2. **`angles.md`** — Polish, newsletter-ready editorial angles in Jakub's voice. Slots directly into the existing `fiteligent-newsletters/` workflow.
...
| Dimension | Decision |
|---|---|
| Output | Two files — English technical digest + Polish publishable angles |
| Frequency | Weekly |
| Schedule | Friday 18:00 Europe/Warsaw |
| Lookback window | 14 days |
| Volume | 15–25 studies per run, theme-grouped, evidence-rated |
| Themes (6) | Obesity & Metabolic Health, Longevity, Strength & Hypertrophy, Supplements, Sleep & Recovery, Skin & Hair Health |
| Approach | Hybrid — APIs/RSS for structured sources + LLM WebSearch for the rest |
| Dedupe | Persistent `seen.json` keyed by DOI + URL, never pruned |
| Storage | Dedicated `fiteligent-research/` git repo |
| Delivery | Git commit + **branded HTML email** to ciastkofski@gmail.com |
| First run | Cold start — first digest will be a fat "state of the field" baseline |
| Execution | Remote scheduled agent (via `mcp__scheduled-tasks__create_scheduled_task`) invoking Python orchestrator |
| Visual identity | Fiteligent brandbook — Satoshi font, dark theme (`#0a0e27`/`#1a1f3a`), `#016FFA` primary blue, `#FF7B00` accent, brand SVG logo |
...
| JAAD (J. Am. Acad. Dermatology) | RSS TOC | Skin & hair theme — keyword match |
...
- **Examine.com** — new articles since last run
- **Stronger by Science / MASS** — Greg Nuckols, Eric Helms
...
- **NSCA / Strength and Conditioning Journal** — TOC scrape
- **Dermatology / skincare / hair-loss voices:** evidence-based dermatologists and skincare/hair-loss researchers — Dr. Shaaf Husaain, Dr. Dray (Andrea Suarez), Dr. Sam Ellis, LabMuffin (Dr. Michelle Wong), Lorraine Massey & others (TBD list — see Section 14)
- **Tressless / Reddit r/tressless** — community-curated AGA research threads (signal-noise risk; flag clearly as low evidence unless they cite primary papers)
...
```
Friday 18:00 Europe/Warsaw  →  Scheduled remote agent fires
                                        ↓
   ┌──────────────────────────────────────────────────┐
   │ 1. COLLECT (hybrid)                              │
   │    Path A: collect_structured.py (APIs/RSS)      │
   │    Path B: collect_unstructured.py (WebSearch)   │
   │    → raw_studies.json                            │
   └──────────────────────────────────────────────────┘
                                        ↓
   ┌──────────────────────────────────────────────────┐
   │ 2. FILTER (filter_dedupe.py)                     │
   │    Load seen.json → drop already-covered items   │
   │    → new_studies.json                            │
   └──────────────────────────────────────────────────┘
                                        ↓
   ┌──────────────────────────────────────────────────┐
   │ 3. SYNTHESIZE (synthesize.py)                    │
   │    LLM theme-groups across 6 themes:             │
   │    obesity / longevity / strength /              │
   │    supplements / sleep / skin & hair             │
   │    Rates evidence, writes:                       │
   │    → digest.md  (English, ~4000-4500 words)      │
   │    → angles.md  (Polish, newsletter-ready)       │
   └──────────────────────────────────────────────────┘
                                        ↓
   ┌──────────────────────────────────────────────────┐
   │ 4. RENDER (render_html.py)                       │
   │    Apply fiteligent brandbook template:          │
   │    → digest.html (branded, dark theme)           │
   │    → angles.html (branded, dark theme)           │
   │    → email_body.html (inlined CSS for clients)   │
   └──────────────────────────────────────────────────┘
                                        ↓
   ┌──────────────────────────────────────────────────┐
   │ 5. DELIVER (notify.py + git)                     │
   │    Update seen.json, git commit + push,          │
   │    email branded HTML digest to ciastkofski@...  │
   └──────────────────────────────────────────────────┘
```

### Why a Python orchestrator instead of a pure-prompt agent

- **Determinism** — same logic each run; debuggable
- **Cost** — only synthesize step needs LLM tokens
- **Testability** — manual trigger, dry-run mode, per-source replay all become trivial
...
├── seen.json                       # dedupe state (DOIs + URLs ever digested)
├── config/
│   ├── topics.yaml                 # MeSH terms + keywords per theme
│   ├── sources.yaml                # API endpoints, RSS URLs, rate limits
│   └── influencers.yaml            # influencer names + canonical URLs/handles
├── scripts/
│   ├── collect_structured.py
│   ├── collect_unstructured.py
│   ├── filter_dedupe.py
│   ├── synthesize.py
│   ├── render_html.py              # markdown → branded HTML via template
│   ├── notify.py
│   ├── run.py                      # orchestrator entry point
│   ├── lib/
│   │   ├── schema.py               # Study dataclass + JSON I/O
│   │   └── evidence.py             # study type → evidence rating
│   └── tests/
│       ├── test_schema.py
│       ├── test_evidence.py
│       ├── test_filter_dedupe.py
│       └── test_integration_goldenpath.py
├── brand/                          # fiteligent brandbook assets (self-contained)
│   ├── template_digest.html        # HTML shell for digest (dark theme)
│   ├── template_angles.html        # HTML shell for angles
│   ├── template_email.html         # email-safe variant (inlined CSS)
│   ├── tokens.css                  # color/font/spacing tokens
│   ├── logo.svg                    # fiteligent wordmark (white on gradient)
│   └── icons/                      # theme icons (obesity, longevity, etc.)
├── digests/
│   └── 2026-05-15/                 # ISO date of run
│       ├── digest.md               # source markdown (canonical)
│       ├── digest.html             # branded HTML render
│       ├── angles.md               # source markdown
│       ├── angles.html             # branded HTML render
│       ├── email_body.html         # inlined-CSS variant sent in email
│       ├── raw_studies.json
│       ├── new_studies.json
│       └── run.log
├── docs/superpowers/specs/
│   └── 2026-05-14-fiteligent-research-routine-design.md
...
Append-only. ~50 KB after a year, ~3 MB after a decade — negligible.
...
Structure:
1. **TL;DR** — top 5 highlights with star ratings
2. **6 theme sections** — Obesity, Longevity, Strength, Supplements, Sleep, Skin & Hair
3. Per study within a section:
   - Title (H3)
   - Evidence stars, authors, journal, DOI link, n, study type
   - 150–200 word breakdown with key numbers (effect size, p, CIs)
   - "Why it matters" — practical implication
   - "Cited by" — cross-reference to any podcast/blog that surfaced it
4. **Sources scanned this week** — counts per source
5. **Run notes** — any failures or partial coverage

### Evidence rating scale (`lib/evidence.py`)

| Rating | Criteria |
|---|---|
| ★★★★★ | Meta-analysis or large RCT (n>500) in high-impact journal |
| ★★★★☆ | Smaller RCT or strong cohort study |
| ★★★☆☆ | Observational with good controls / small RCT |
| ★★☆☆☆ | Preprint, mechanistic, or weak observational |
| ★☆☆☆☆ | Opinion, anecdote, podcast claim without cited source |

### `angles.md` (Polish, newsletter-ready)

Front matter: date, language, link back to digest.

Structure: 5–10 numbered angles. Each angle:
- **Hook headline** (Polish, in Jakub's voice — sarcastic, no BS)
- **Pitch** (2–3 sentences, conversational)
- **Suggested CTA** (Polish)
- **Źródło w digest** — pointer to the digest entry that inspired it
- **Zalecany kąt newslettera** — mapping to one of the 10 newsletter angle slots from `fiteligent-newsletters/TEMPLATE.md`
...
The digest container is **720px** (vs. 600px for newsletters) — research content is denser and reads better with more horizontal room. Newsletter angles stay at 600px to match the existing newsletter ecosystem.
...
| `email_body.html` | 600px | Email-client-safe: tables-based layout, inlined CSS (via `premailer` or equivalent), one screen of TL;DR + "open full digest →" CTA linking to `digest.html` on disk/repo |
...
- **Email header:** white wordmark on the `#0047CC → #016FFA → #66A8FF` gradient (same as `DARK_TEMPLATE_BASE.html`)
...
2. Parse digest.md → AST (via markdown-it-py)
3. Map AST → template blocks: TL;DR cards, theme sections, study cards, footer
4. Inject theme accent colors per section
5. Write digest.html
6. Same flow for angles.md → angles.html (using template_angles.html)
...
| `DARK_TEMPLATE_BASE.html` | `brand/template_email.html` | Adapt — replace `{{CONTENT}}` with Jinja2 blocks for digest TL;DR + theme sections |
| (derived from brandbook PDF colors) | `brand/tokens.css` | Already specified above |

The remaining theme icons (`obesity.svg`, `longevity.svg`, `supplements.svg`, `sleep.svg`, `skin-hair.svg`) need to be either sourced from the brandbook PDF or commissioned. **Open question** — see Section 14.
...
2. collect_structured()   → list[Study]
3. collect_unstructured() → list[Study]
4. merge + filter_dedupe() against seen.json
5. if len(new_studies) < 5: send "thin week" alert and abort gracefully
6. synthesize() → digest.md + angles.md
7. render_html() → digest.html + angles.html + email_body.html
8. update seen.json with new DOIs/URLs
9. git add + commit "weekly digest YYYY-MM-DD (N studies)" + push
10. notify() → send email_body.html to ciastkofski@gmail.com (with repo commit link)
...
| Render fails | Markdown succeeded — commit anyway, email plain-text body with `digest.md` attached; subject tagged `RENDER_FAILED` |
...
- `[Fiteligent Research] Digest 2026-05-15 (22 studies)` — normal
- `[Fiteligent Research] PARTIAL 2026-05-15 (8 studies, see notes)` — degraded
- `[Fiteligent Research] FAILED 2026-05-15` — full failure with diagnostics
- `[Fiteligent Research] THIN WEEK 2026-05-15` — < 5 new studies
...
| Command | Purpose |
|---|---|
| `python scripts/run.py` | Default — what the cron fires |
| `python scripts/run.py --manual` | Same as default but tagged "manual" in logs/email |
| `python scripts/run.py --dry-run` | Full pipeline → `digests/_dry/<timestamp>/`; no commit, no email |
...
**Tested:**
- `lib/schema.py` — Study dataclass JSON round-trip
- `lib/evidence.py` — study type → star rating table-driven
- `filter_dedupe.py` — given known `seen.json` + input, drops the right items
- `render_html.py` — given canonical `digest.md` fixture → produces HTML matching golden file (smoke test: contains expected theme sections, accent colors are applied, no broken Jinja blocks)
- One **golden-path integration test** with mocked API responses → assert `digest.md` is generated with expected structure (sections, counts, evidence ratings)

**Not tested:**
- Live API calls (network-brittle)
- LLM synthesis output (non-deterministic — covered by human review in rollout)
...
**Phase 1 — Manual dry runs (week 1)**
...
**Phase 2 — Manual trigger, real output (week 2)**
...
- **Per-run:** `digests/<date>/run.log` — per-source counts, errors, timing
...
- **Theme icons (`brand/icons/obesity.svg`, `longevity.svg`, `supplements.svg`, `sleep.svg`, `skin-hair.svg`)** — `strength.svg` reuses the existing `LOGO_HANTEL.svg`, but the other five don't have brand-native equivalents. Options: (a) source from the brandbook PDF if present, (b) commission/design new ones in fiteligent style, (c) ship v1 with text-only theme headers and add icons in v1.1. Recommended default: **(c)** — don't block rollout on icons.
- **Dermatology / skincare / hair-loss influencer list** — initial Section 3 list (Dr. Husaain, Dr. Dray, Dr. Sam Ellis, LabMuffin) is provisional. Before Phase 2 rollout, vet candidates against the "evidence-grade signal vs. opinion noise" bar applied to the core 15 influencers. Drop pure-aesthetic/beauty voices that don't cite primary literature.

**Items intentionally deferred from scope:**

- **Polish-language influencer coverage** (Parol, Ostręga, Mauricz) — deferred per user decision; can be added later by extending `influencers.yaml`
- **Notion / Drive cross-posting** — explicitly rejected; git + email only
- **Web dashboard / published research blog** — not in scope; the HTML files in the repo are the UI. If you later want a public research page on fiteligent.pl, the HTML output is already styled correctly and can be lifted directly.
- **Per-angle A/B newsletter analytics feedback loop** — interesting but out of scope for v1
- **Polish translation of `digest.md`** — synthesize step writes English digest only; if you want a Polish translation later, add a `translate.py` step between synthesize and render
...
2. ≥ 80% of digest studies are *new* (dedupe is working)
3. Each digest produces ≥ 3 angles that actually become newsletter content over the following month
...
5. No silent failures — every degraded run produces a clearly-tagged email
